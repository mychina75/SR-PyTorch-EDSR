...
class DRLN(nn.Module):
    def __init__(self, args):
        ...
        self.lam = LAM_Module(chs)
        self.c27 = ops.BasicBlock(chs*6, chs, 3, 1, 1)

        self.csa = CSAM_Module(chs*6)
        self.c28 = ops.BasicBlock(chs * 6, chs, 3, 1, 1)

        self.c29 = ops.BasicBlock(chs * 2, chs, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        cat_layer = torch.cat([a1.unsqueeze(1), a2.unsqueeze(1), a3.unsqueeze(1), a4.unsqueeze(1),
                               a5.unsqueeze(1), a6.unsqueeze(1)], dim=1)

        cat_layer2 = torch.cat([a1, a2, a3, a4,
                               a5, a6], dim=1)

        a7 = self.lam(cat_layer)
        a7 = self.c27(a7)

        a8 = self.csa(cat_layer2)
        a8 = self.c28(a8)

        a9 = torch.cat([a7, a8], dim=1)
        a9 = self.c29(a9)
