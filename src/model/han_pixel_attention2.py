...
class PALayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.c3 = ops.BasicBlock(channel, channel, 3, 1, 1, 1)
        self.c4 = ops.BasicBlock(channel, channel // reduction, 3, 1, 1, 1)
        self.c5 = ops.BasicBlock(3, 1, 1, 1, 0, 1)
        self.c6 = ops.BasicBlockSig(channel // reduction, channel, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        c5 = torch.cat([c1.view(m_batchsize, 1, C // self.reduction * height * width, 1),
                           c2.view(m_batchsize, 1, C // self.reduction * height * width, 1),
                           c4.view(m_batchsize, 1, C // self.reduction * height * width, 1)], dim=1)
        c5 = self.c5(c5)

        a = c5.view(m_batchsize, C // self.reduction, height, width)
        y = self.c6(a)
...
class RCAB(nn.Module):
    def __init__(
        ...
        modules_body.append(PALayer(n_feat, reduction))
