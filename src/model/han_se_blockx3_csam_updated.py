...
class CSAM_Module(nn.Module):
    ...
    def __init__(self, in_dim, conv):
        super(CSAM_Module, self).__init__()
        self.chanel_in = in_dim

        self.conv3 = nn.Conv3d(1, 1, 3, 1, 1)
        self.conv5 = nn.Conv3d(1, 1, 5, 1, 2)
        self.conv7 = nn.Conv3d(1, 1, 7, 1, 3)

        # self.gamma3 = nn.Parameter(torch.zeros(1))
        # self.gamma5 = nn.Parameter(torch.zeros(1))
        # self.gamma7 = nn.Parameter(torch.zeros(1))
        self.gamma = nn.Parameter(torch.zeros(1))

        self.conv_fuse = conv(in_dim*3, in_dim, 1)
    ...
    def forward(self, x):
        ...
        out3 = out3.view(m_batchsize, -1, height, width)
        out5 = out5.view(m_batchsize, -1, height, width)
        out7 = out7.view(m_batchsize, -1, height, width)

        # out3 = self.gamma3 * out3
        # out5 = self.gamma5 * out5
        # out7 = self.gamma7 * out7
...
class HAN(nn.Module):
    def __init__(self, args, conv=model.common.default_conv):
        ...
        self.body = nn.Sequential(*modules_body)
        self.csa = CSAM_Module(n_feats,conv)
